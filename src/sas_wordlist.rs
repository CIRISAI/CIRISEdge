//! The BIP-39 English wordlist (2048 words), vendored (CIRISEdge#264).
//!
//! `src/sas.rs` needs only this static `&str` table for its SAS
//! (short-authentication-string) encoding — not a full BIP-39
//! mnemonic/bitcoin crate. Vendoring it drops `bip39` and its
//! bitcoin-adjacent transitives (`bitcoin_hashes`, `hex-conservative`,
//! `arrayvec`, `unicode-normalization`) — the only wallet-adjacent crates
//! in the tree — for a ~13 KB static table.
//!
//! The list is the canonical BIP-39 English wordlist (public domain; the
//! `bip39` crate ships it under CC0-1.0). Sorted, exactly 2048 unique
//! words, "abandon" first, "zoo" last. A round-trip test in `sas.rs`
//! pins length + boundaries + uniqueness against any drift.
...
mod tests {
    ...
    /// CIRISEdge#264 — pin the vendored list against silent drift: the
    /// canonical BIP-39 English wordlist is exactly 2048 unique lowercase
    /// words, "abandon" first and "zoo" last, and lexicographically sorted.
    /// SAS encoding indexes it by an 11-bit value (2^11 = 2048), so a wrong
    /// length or order would silently corrupt every short-auth string.
    #[test]
    fn wordlist_is_canonical_bip39_english() {
        assert_eq!(
            ENGLISH.len(),
            2048,
            "11-bit SAS index requires exactly 2048"
        );
        assert_eq!(ENGLISH[0], "abandon");
        assert_eq!(ENGLISH[2047], "zoo");
        let unique: HashSet<&&str> = ENGLISH.iter().collect();
        assert_eq!(unique.len(), 2048, "all words unique");
        assert!(
            ENGLISH.windows(2).all(|w| w[0] < w[1]),
            "wordlist must be lexicographically sorted + deduplicated"
        );
        assert!(
            ENGLISH
                .iter()

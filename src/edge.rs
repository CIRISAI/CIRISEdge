...
use crate::messages::{is_federation_attestation_emitting_type, EdgeEnvelope, MessageType};
use crate::outbound::{
...
impl Edge {
    ...
    /// Test-only: drive a single [`InboundFrame`] through the full
    /// `dispatch_inbound` pipeline (verify + ACK match +
    /// FederationAnnouncement/StewardDirective attestation emission +
    /// InlineText fan-out + typed handler dispatch). Exists so the
    /// CIRISEdge#20 acceptance suite can exercise the receiver-side
    /// attestation-emission hook without running the full
    /// listener-loop machinery in [`Self::run`]. Production callers
    /// reach the same code via [`Self::run`]; this entry point is
    /// purely a synchronous test convenience.

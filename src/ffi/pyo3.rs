...
//!
//! # v0.13.1 — local_signer_capsule (CIRISPersist#119, CIRISEdge#43)
//!
//! v0.13.0 read the Reticulum-transport federation Ed25519 pubkey from
//! the `keyring_signer_capsule` (the hardware-rooted hybrid signer).
//! Under `keyring_storage_kind = hardware_hsm_only` the hardware path
//! emits a 65-byte hybrid pubkey (e.g. TPM P-256), and
...
//! deployment.
//!
//! v0.13.1 consumes persist v3.1.1's `local_signer_capsule` — a fifth
...
///      keyring storage)
/// 2. Edge's `Cargo.toml` pins `ciris-persist = "3", tag = "v3.1.1"`
///    and pyproject.toml pins `ciris-persist>=3.1.1,<4` — together,
...
/// v0.13.1 (CIRISEdge#43 / CIRISPersist#119) — typed outcome of the
/// `local_signer_capsule` extraction attempt. Only [`init_edge_runtime`]
/// consumes this; the variants drive the fallback-vs-hard-error
/// decision in Step 3.5.
#[cfg(feature = "transport-reticulum")]
...
/// v0.13.1 (CIRISEdge#43 / CIRISPersist#119) — extract the persist
/// transport-identity `Arc<LocalSigner>` from the shared engine's
/// `local_signer_capsule()` pymethod.
///
/// Returns:
...
/// `local_signer_unavailable` ValueError) that the other four
/// capsules do not — for the other four, a missing method is a hard
/// "wrong persist version" error.
#[cfg(feature = "transport-reticulum")]
#[allow(unsafe_code)]
fn extract_local_signer(
    ...
    // SAFETY: persist v3.1.1's `local_signer_capsule_py`
    // (`src/ffi/pyo3.rs` line ~14718) wraps a
    // `Arc<ciris_persist::signing::LocalSigner>` under the name tag
    // `"ciris_persist::local_signer"`. The Cargo pin
    // (`ciris-persist = { tag = "v3.1.1", version = "3" }`) enforces
    // that contract. The cloned `Arc` is owned by the caller — the
    // capsule reference doesn't escape this function.
    #[allow(deprecated)]
...
///   (`docs/COHABITATION.md` rule 1). This drives the hot-path scrub-
///   envelope signing surface (`Edge::send_durable`).
/// - v0.13.1 (CIRISEdge#43): Calls `engine.local_signer_capsule()` →
///   extracts `Arc<ciris_persist::signing::LocalSigner>`, wraps it in
...
///   configured hybrid policy. The transport's authenticated
///   cold-start path (CIRISEdge#15, AV-42 closure) is then live.
...
fn init_edge_runtime(
    ...
    // signer (CIRISEdge#43 / CIRISPersist#119, v0.13.1).
    //
        ...
        Err(LocalSignerCapsuleError::MethodAbsent) => {
            tracing::warn!(
                "ciris_persist.Engine does not expose local_signer_capsule() \
                 ...
                 hardware_hsm_only this fallback will fail at \
                 ReticulumTransport::new with 'federation Ed25519 pubkey \
                 must be 32 bytes, got 65'. Upgrade ciris-persist (Cargo \
                 `tag = \"v3.1.1\"`, pyproject `ciris-persist>=3.1.1,<4`) \
                 to consume CIRISPersist#119."
            );
            signer.clone()
        }
    ...
    let auth = ReticulumAuth {
        // v0.13.1 (CIRISEdge#43): the Reticulum-identity signer is
        // split out from the hot-path keyring signer above. When
        // local_signer_capsule is available
...
mod pyo3_tier2_tests {
            ...
            Err(e) => {
                ...
                assert!(
                    ...
                    "init_edge_runtime must reach past capsule extraction with a \
                     ...
                     v0.13.1 adds local_signer_capsule for CIRISEdge#43); \
                     got: {msg}"
                );
    ...
    // ── v0.13.1 (CIRISEdge#43 / CIRISPersist#119) — local_signer_capsule
    //
    // The federation-cohab unblocker for CIRISAgent 2.9.4: v0.13.0 read
    // the Reticulum transport-identity Ed25519 pubkey from
    // `keyring_signer_capsule`, which under
    ...
    // transport-identity `Arc<LocalSigner>`; v0.13.1 routes the
    ...
    // hard check (`src/transport/reticulum.rs` line ~1811); any
    // regression there would surface as a `federation Ed25519 pubkey
    // must be 32 bytes` error message.

    /// v0.13.1 happy path — engine constructed with `local_key_id` +
    /// `local_key_path` exposes a populated `local_signer_capsule()`.
    /// `init_edge_runtime` routes the Reticulum-identity signer through
    /// `LocalSignerHardwareAdapter` and the transport builds cleanly
    /// (the 32-byte Ed25519 raw pubkey from `LocalSigner.signing_key
    /// .verifying_key().to_bytes()` satisfies
    /// `ReticulumTransport::new`'s identity validation, regardless of
    /// what the platform keyring's `signing_key_id` resolves to —
    /// hardware P-256 included).
    ///
    ...
    fn py_init_edge_runtime_local_signer_capsule_supplies_reticulum_identity() {
        ...
        let outcome: Result<String, PyErr> = Python::attach(|py| -> PyResult<String> {
            ...
            // Cargo pin (v3.1.1) didn't take — fail loudly so the
        ...
        match outcome {
            ...
            Err(e) => {
                ...
                // exact shape v0.13.1 is meant to close. Capsule
                // extraction itself must also not surface as the
                // failure point.
                let msg = e.to_string();
                assert!(
                    !msg.contains("32 bytes, got 65"),
                    "v0.13.1 must close the v0.13.0 'federation Ed25519 pubkey \
    ...
    /// v0.13.1 fallback path — engine constructed WITHOUT `local_key_id`
    /// (and without `local_key_path`) raises the typed
    /// `ValueError("local_signer_unavailable")` from
            ...
            Err(e) => {
                ...
                assert!(
                    ...
                    "local_signer_unavailable ValueError MUST be caught + drive \
                     ...
                     bug v0.13.1's typed-error-detection branch closes. Got: {msg}"
                );

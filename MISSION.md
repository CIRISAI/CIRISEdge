...
| CC 4.4.3.4.3 / 3.4.7.3 | Infra-vs-agency split — a key whose `identity_type` CONTAINS `node` performs §19.7 forever-memory without reasoning; brain enrichment optional. Re-pointed from CC 1.13.5 at persist v38.6.0 (CIRISPersist#773): 1.13.5 is the operational-language gate cited above, and the infra-vs-agency rule graduated to CC 3.4.7.3. Clause B is now MEMBERSHIP, not exclusivity — "pure fabric node" was the escape hatch (fusing `agent` onto a node key silently repealed the property), so the gate fires on `{node,agent}` hybrids too |
